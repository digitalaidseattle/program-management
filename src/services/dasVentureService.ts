...
class DASVentureService {
    ...
    getAll = async (): Promise<any[]> => {
        return airtableService.getTableRecords(PROJECT_TABLE, MAX_RECORDS, FILTER)
    ...
    getById = async (id: string): Promise<any> => {
        return airtableService.getRecord(PROJECT_TABLE, id)
            .then(r => {
                return {
                    id: r.id,
                    name: r.fields['Project Name'],
    ...
    getEpics = async (project: any): Promise<any> => {
        return Promise.all(project.epicIds
            .map((e: string) => airtableService.getRecord(EPIC_TABLE, e)))
            .then((resps: any) =>
                resps.map((r: any) => {
                    return {
                        id: r.id,
    ...
    getContributors = async (project: any): Promise<any> => {
        return Promise.all(project.contributorIds
            .map((e: string) => airtableService.getRecord(CONTRIBUTOR_TABLE, e)))
            .then((resps: any) =>
                resps.map((r: any) => {
                    return {
                        id: r.id,
    ...
    getSprints = async (project: any): Promise<any> => {
        return Promise.all(project.sprintIds
            .map((e: string) => airtableService.getRecord(SPRINT_TABLE, e)))
            .then((resps: any) =>
                resps.map((s: any) => {
                    return {
                        id: s.id,
    ...
    getFeatures = async (epic: any): Promise<any> => {
        return Promise.all(epic.featureIds
            .map((e: string) => airtableService.getRecord(FEATURE_TABLE, e)))
            .then((resps: any) =>
                resps.map((f: any) => {
                    return {
                        id: f.id,
    ...
    getStories = async (feature: any): Promise<any> => {
        return Promise.all(feature.storyIds
            .map((s: string) => airtableService.getRecord(STORY_TABLE, s)))
            .then((resps: any) =>
                resps.map((s: any) => {
                    return {
                        id: s.id,
    ...
    getTasks = async (story: any): Promise<any> => {
        return Promise.all(story.taskIds
            .map((t: string) => airtableService.getRecord(TASK_TABLE, t)))
            .then((resps: any) =>
                resps.map((t: any) => {
                    return {
                        id: t.id,
    ...
    addSprint = async (): Promise<any> => {
        ...
        return airtableService.createRecord('tbliVWHEcbUD1sVVA', sprint)
            .then(resp => resp)
    }
}
...
export type { VentureProps, SprintProps };